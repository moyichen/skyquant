...
class ManualTradeReview:
    def __init__(self, config_path="config.yaml", trade_csv="manual_trades.csv"):
        ...
        self.trade_df = pd.read_csv(
            trade_csv, parse_dates=["trade_date"], dtype={"stock_code": str}
        )
        missing = [c for c in REQUIRED_COLS if c not in self.trade_df.columns]
        if missing:
            raise ValueError(
                f"manual_trades.csv is missing required columns: {missing}, existing columns: {list(self.trade_df.columns)}, standard format is trade_date,stock_code,side,price,size,profit_loss"
            )
        self.result_list = []
    ...
    def get_strategy_signal(self, code, strategy_id, param):
        ...
        if len(trades_df) > 0:
            for _, t in trades_df.iterrows():
                sig.append(
                    {"date": t["entry_date"], "side": "BUY", "price": t["entry_price"]}
                )
                sig.append(
                    {"date": t["exit_date"], "side": "SELL", "price": t["exit_price"]}
                )
    ...
    def match_manual_trade(self):
        ...
        strategy_params = {
            str(code): p
            for code, p in (self.cfg.get("strategy_params", {}) or {}).items()
        }
        for _, row in self.trade_df.iterrows():
            ...
            # Read the optimized strategy parameters for this symbol
                ...
                self.result_list.append(
                    {
                        ...
                        "match": False,
                        "note": "No strategy config",
                    }
                )
                continue
            for strategy_id, param in strategy_params[code].items():
    ...
    def summary_report(self, out_csv="output/manual_review_result.csv"):
        ...
        if total_cnt == 0:
            print(
                "No review results (manual trades have no corresponding strategy config or no market data), empty report output"
            )
            return df
        ...
        if len(profit_loss_df) > 0:
            win_rate = (
                profit_loss_df[profit_loss_df["manual_profit_loss"] > 0].shape[0]
            ) / len(profit_loss_df)
        ...
        print(
            f"Matches with strategy signals: {match_cnt}, match rate: {match_rate:.2%}"
        )
        print(
            f"Manual trade win rate: {win_str} (based on {len(profit_loss_df)} trades with profit-loss records)"
        )
