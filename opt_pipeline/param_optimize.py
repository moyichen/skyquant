...
from common import BacktestRunner, PARAM_GRID_CSV
...
def main():
    ...
    result_rows = []
    for strat_id, grid in PARAM_GRID.items():
        keys = list(grid.keys())
        for param_tuple in itertools.product(*grid.values()):
            ...
            for code in valid_codes:
                ...
                if df is None or df.empty:
                    ...
                    continue
                try:
                    final_value = runner.run(df, strat_id, param_dict)
                    ...
                    print(f"异常 {code} {strat_id} {param_dict}:{e}")
