...
from common import (extract_params, to_native, read_stage_csv,
                    AGGREGATE_CSV, CONFIG_PATH)
...
def load_config():
    ...
    if not os.path.exists(CONFIG_PATH):
        return {"global_setting": {}, "commission_config": {},
                "stock_list": [], "strategy_params": {}}
...
def main():
    ...
    config["strategy_params"] = {str(k): v for k, v in config["strategy_params"].items()}
    ...
    for _, row in valid_df.iterrows():
        ...
        config["strategy_params"][code][strat_id] = param
    ...
    for code, strat_item in config["strategy_params"].items():
        for sname, p in strat_item.items():
