...
def run_step(name, cwd, cmd):
    ...
    try:
        proc = subprocess.Popen(
            cmd, cwd=str(cwd), stdout=sys.stdout, stderr=sys.stderr, text=True
        )
        proc.wait()
        if proc.returncode != 0:
            raise RuntimeError(
                f"Subprocess returned non-zero exit code: {proc.returncode}"
            )
...
def main():
    ...
    if not args.skip_data:
        ...
        logger.info(
            "👉 --skip-data enabled, skipping market data fetch, using local cache"
        )
    ...
    STEPS += [
        (
            ...
            OPT_DIR,
            [sys.executable, "param_optimize.py"],
        ),
        (
        ...
        (
            ...
            BASE_DIR,
            [sys.executable, "manual_trade_review.py"],
        ),
    ]
