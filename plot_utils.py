...
def plot_equity_drawdown(equity_df:pd.DataFrame, metrics:dict, save_path:str):
    ...
    ax1.set_title(f"净值曲线 | 年化:{metrics['annual_return']:.2%} 夏普:{metrics['sharpe_ratio']:.2f} 最大回撤:{metrics['max_drawdown']:.2%}")
    ax1.set_ylabel("资产")
    ax1.legend()
    ax1.grid(alpha=0.3)

    ax2.plot(equity_df["datetime"], equity_df["drawdown"], color="#A23B72", label="回撤")
...
def plot_win_pie(metrics:dict, save_path:str):
    ...
    ax.pie([win, lose], labels=[f"盈利 {win:.1f}%", f"亏损 {lose:.1f}%"], colors=["#57CC99","#F38181"], autopct="%.1f%%")
    ax.set_title(f"交易胜率饼图 | 总交易次数:{metrics['total_trades']},盈亏比:{metrics['profit_factor']:.2f}")
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()

def plot_all(equity_df, trades_df, metrics, out_dir, code, strat_name):
    os.makedirs(out_dir, exist_ok=True)
    eq_path = os.path.join(out_dir, f"{code}_{strat_name}_equity_dd.png")
    pie_path = os.path.join(out_dir, f"{code}_{strat_name}_win_pie.png")
